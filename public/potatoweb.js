/* POTATO-OS — PotatoWeb Browser + The Peeler Settings
   FIXED: blob URL for proxied content (prevents same-origin frame attacks)
   FIXED: srcdoc for error pages
   FIXED: proper proxy fetch with timeout
*/
"use strict";
...
const TuberNet = (() => {
  ...
  function favUrl(url){ if(!url||url==="_nt") return ""; try{ const u=new URL(/^https?:\/\//i.test(url)?url:"https://"+url); return `https://www.google.com/s2/favicons?domain=${u.hostname}&sz=14`; } catch{ return ""; } }
  ...
  function renderTabs(){
    ...
    strip.innerHTML = tabs.map(t => `
      <div class="tn-tab ${t.id===activeId?"active":""}" onclick="TuberNet.switchTab(${t.id})">
        <img class="tn-tab-fav" src="${favUrl(t.url)}" onerror="this.style.display='none'" alt="">
  ...
  /* show/hide helpers */
  function vis(id, show){ const e=document.getElementById(id); if(e) e.classList.toggle("show", show); }
  function showNT()  { vis("tn-nt",true);  vis("tn-load",false); vis("tn-frame",false); setStatus("","Ready"); }
  function showLoad(){ vis("tn-nt",false); vis("tn-load",true);  vis("tn-frame",false); setStatus("loading","Connecting..."); }
  function showPage(){ vis("tn-nt",false); vis("tn-load",false); vis("tn-frame",true); }
  ...
  /* ── FIXED navigate — blob URL for proxy content ── */
  async function navigate(raw){
    ...
    const tab = active(); if(tab){ tab.url=url; tab.title=dom(url); }
    const urlEl = document.getElementById("tn-url"); if(urlEl) urlEl.value = url;
    ...
    try {
      ...
      if(!html || html.length < 10) throw new Error("Empty response");

      const frame = document.getElementById("tn-frame");
      if(frame){
        showPage();
        /* Use blob URL — cross-origin isolation prevents the loaded page
           from navigating the parent OS window */
        const blob    = new Blob([html], { type: "text/html" });
        const blobUrl = URL.createObjectURL(blob);
        frame.onload  = () => URL.revokeObjectURL(blobUrl);
        frame.src     = blobUrl;
        setStatus("ok", "Loaded · " + dom(url));
        if(tab) tab.title = dom(url);
        renderTabs();
      }
    } catch(err){
      /* Proxy failed — try direct iframe as fallback */
      const frame = document.getElementById("tn-frame");
      if(frame){
        showPage();
        frame.src = url;
        frame.onload  = () => setStatus("ok", dom(url));
        frame.onerror = () => showErr(url);
        setStatus("loading", "Direct load...");
      }
    }
  }
  ...
  function showErr(url){
    ...
    frame.srcdoc = `<html><body style="background:#070605;color:#f5e6c8;font-family:system-ui;display:flex;flex-direction:column;align-items:center;justify-content:center;height:100vh;gap:12px;margin:0">
      <div style="font-size:52px">⚡</div>
      <p style="font-size:17px;font-weight:700">Can't reach this page</p>
      <p style="font-size:12px;opacity:0.45">${url}</p>
      <button onclick="window.open('${url}','_blank')" style="background:rgba(212,168,83,0.15);border:1px solid rgba(212,168,83,0.3);color:#f5e6c8;padding:9px 22px;border-radius:7px;cursor:pointer;font-size:13px;margin-top:8px">
        Open in new window
      </button>
    </body></html>`;
  ...
  function injectStyles(container){
    ...
    #tn-nt::before{content:'';position:absolute;inset:0;background:rgba(0,0,0,0.28);pointer-events:none}
...
const Peeler = (() => {
  ...
  function build(el, cfg){
    ...
    const navs = [
      {i:"fa-palette",l:"Appearance"},{i:"fa-globe",l:"Network"},
      {i:"fa-store",l:"Spud Store"},{i:"fa-bell",l:"Notifications"},
      {i:"fa-shield-alt",l:"Privacy"},{i:"fa-microchip",l:"System"},
      {i:"fa-info-circle",l:"About"},
    ];
    el.innerHTML = `<div id="peeler">
      <div class="pl-sidebar">
        <div class="pl-logo">THE PEELER</div>
        ${navs.map((n,i) => `
          <div class="pl-nav${i===0?" on":""}" onclick="Peeler.tab(${i},this)">
            <i class="fas ${n.i}"></i> ${n.l}
          </div>`).join("")}
      </div>
      <div class="pl-content" id="pl-content"></div>
    </div>`;
    makeTabs();
    tab(0, el.querySelector(".pl-nav.on"));
  }
    ...
    _tabs = [
      () => `<div class="page-title">APPEARANCE</div>
        ...
        <div class="pl-card"><div class="pl-card-hdr">Accent Colour</div>
          <div class="pl-row" style="border:none"><div class="pl-dots">
            ${["#D4A853","#5BB974","#5B8FD4","#E05252","#9B72CF","#F07850"].map(c =>
              `<div class="pl-dot" style="background:${c}" onclick="document.documentElement.style.setProperty('--amber','${c}');OS.notify('Appearance','Accent updated!','🎨')"></div>`
            ).join("")}
          </div></div>
        </div>`,
      () => `<div class="page-title">NETWORK</div>
        <div class="pl-card"><div class="pl-card-hdr">PotatoWeb Proxy</div>
          ...
          ${iv("Method","node-fetch → strips X-Frame-Options + CSP")}
          ${iv("Game Fetch","/game?id=N (CDN relay)")}
          ${iv("Zones","/zones (GN-Math catalog)")}
          <div class="pl-row" style="border:none"><div><h4>Test Proxy</h4><p>Check server connection</p></div>
      ...
      () => `<div class="page-title">PRIVACY</div>
        ...
        <div class="pl-card"><div class="pl-card-hdr">Danger Zone</div>
          <div class="pl-row" style="border:none"><div><h4>Wipe & Reboot</h4><p>Clears all data and reloads</p></div>
            <button class="btn-ghost btn-sm" style="color:var(--red);border-color:var(--red)"
                    onclick="if(confirm('Wipe all data?')){localStorage.clear();location.reload()}">Wipe</button></div>
        </div>`,
      () => `<div class="page-title">SYSTEM</div>
        <div class="pl-card"><div class="pl-card-hdr">Runtime</div>
          ${iv("OS","Potato-OS v3.1.0")}${iv("Build","GOLDEN-HARVEST-2025")}
          ${iv("Storage Used",(JSON.stringify(localStorage).length/1024).toFixed(1)+" KB")}
        </div>
        <div class="pl-card"><div class="pl-card-hdr">Display</div>
          <div class="pl-row"><div><h4>FPS Counter</h4><p>Show frame rate</p></div>
          <label class="os-tog"><input type="checkbox" checked onchange="document.getElementById('fps').style.display=this.checked?'block':'none'"><span class="tog-t"></span></label></div>
        </div>`,
      () => `<div class="page-title">ABOUT</div>
        <div style="text-align:center;padding:24px 0">
          ...
          <div class="pl-card" style="text-align:left;max-width:380px;margin:0 auto">
            ${iv("Games","GN-Math (gn-math.dev)")}
            ${iv("Browser","PotatoWeb (server-side proxy)")}
            ${iv("Music","Spudify (Spotify Web)")}
            ${iv("License","Spud Public License v3")}
          </div>
        </div>`,
    ];
  }
  ...
  function tab(i, el){
    document.querySelectorAll(".pl-nav").forEach(n => n.classList.remove("on"));
    if(el) el.classList.add("on");
    const c = document.getElementById("pl-content");
    if(c && _tabs[i]) c.innerHTML = _tabs[i]();
  }

  async function testProxy(){
    OS.notify("Network","Testing proxy...","📡");
    try{
      const r = await fetch("/proxy?url=https://httpbin.org/get", { signal: AbortSignal.timeout(8000) });
      if(r.ok) OS.notify("Network","Proxy working ✓","✅");
      else throw new Error("HTTP " + r.status);
    } catch(e){ OS.notify("Network","Proxy error: " + e.message,"❌"); }
  }

  function clearGames(){
    if(!confirm("Remove all installed games?")) return;
    Object.keys(localStorage).filter(k => k.startsWith("pot_html_") || k === "pot_installed").forEach(k => localStorage.removeItem(k));
    OS.notify("Spud Store","All games cleared.","🗑️");
  }
...
window.PotatoWeb = TuberNet; // alias for HTML onclick calls
